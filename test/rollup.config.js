      ...
      plugins: [
        {
          async writeBundle() {
            ...
            const corruptedmap = bundle.replace(
              /(\r?\n\/\/#\s*sourceMappingURL=bundle).+$/m,
              '$1-corruptedmap.js.map'
            );
      ...
      plugins: [
        {
          async writeBundle() {
            ...
            const map =
              /\r?\n\/\/#\s*sourceMappingURL=data:application\/json;charset=utf-8;base64,(.+)$/m.exec(
                shared
              )[1];
            // Create a raw (URI-encoded) source map
            const raw = shared.replace(
            ...
            const invalid = shared.replace(
              /(\r?\n\/\/#\s*sourceMappingURL=data:application\/json).+$/m,
              `$1;${encodeURIComponent(Buffer.from(map, 'base64').toString())}`
            );
            await writeFile('out/shared-invalid.js', invalid);
            // Corrupt the source map content
            const corrupted = shared.replace(
              /(\r?\n\/\/#\s*sourceMappingURL=data:application\/json).+$/m,
